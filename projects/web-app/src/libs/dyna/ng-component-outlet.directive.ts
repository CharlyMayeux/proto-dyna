//https://stackblitz.com/edit/angular-next-nuooxs?file=src%2Fmain.ts,src%2Fcomponent-outlet.directive.ts
/**
 * @license
...
export class NgComponentOutlet implements OnChanges, DoCheck, OnDestroy {
  ...
  private _outputsUsed = new Map<string, [(...args: any[]) => unknown, { unsubscribe: () => void } | null]>();
  ...
  private _needToReCreateNgModuleInstance(changes: SimpleChanges): boolean {
    ...
    // were changed).
    return (
      changes['ngComponentOutletNgModule'] !== undefined || changes['ngComponentOutletNgModuleFactory'] !== undefined
    );
  }
  ...
  private _applyOutputs() {
    ...
    for (const [outputName, callback] of Object.entries(outputs ?? {})) {
      ...
      const prop = (this.ngComponentOutlet! as any).ɵcmp.outputs[outputName];
      console.log(outputName, (this.ngComponentOutlet! as any).ɵcmp.outputs, prop);
      const maybeEmitter = this._componentRef?.instance[prop];
